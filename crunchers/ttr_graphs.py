...
def plot_distribution(frame):
    df = frame.loc[(frame['name'] == 'policy_recovery_time')]
    df.loc[:,quantiles] = df.loc[:,quantiles].mul(1e-9)
    table = df.pivot_table(index="GraphSize", values=quantiles)
    ax = table.plot()
...
def plot_mean(frame):
    ...
    table = df.pivot_table(index="GraphSize", values="quantiles.0.5", columns="ExpLabel")
